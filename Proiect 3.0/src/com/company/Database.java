...
public class Database {
     ...
     * @implSpec The class must have exactly one constructor annotated with "@ID" which takes any number of String parameters (also checked in TableRow).
     */
            ...
            for (Constructor<?> ctor: cls.getConstructors()) {
                for (Annotation annotation: ctor.getDeclaredAnnotations()) {
                    if (annotation.annotationType().equals(ID.class)) {
                        if (constructor != null) { throw new RuntimeException(cls.getSimpleName()+" has multiple constructors annotated with @ID"); }
                        constructor = ctor;
                    }
                }
            }
            if (constructor == null) { throw new RuntimeException(cls.getSimpleName()+" has no constructor annotated with @ID"); }
            Set<Class<?>> parameterTypes = new HashSet<>(Arrays.asList(constructor.getParameterTypes()));
            if (!(parameterTypes.size() == 0 || (parameterTypes.contains(String.class) && parameterTypes.size() == 1))) {
                throw new RuntimeException(cls.getSimpleName()+"'s @ID constructor doesn't take only strings as parameters");
            }
    ...
    /**
     * Loads data from a file. The order of the lines doesn't matter as long as every entry has the correct flag associated.
